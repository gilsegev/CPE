...
6. The system should have an ability to bill the users for the content. Users pay directly to my wife using any existing payment methodology (Square or something else), and then handle expenses with CPE later. This requires a way to expense or collect billing.  
   1. The Stripe integration must be configured to automatically trigger a highly detailed, itemized email receipt upon checkout. If possible, the Next.js frontend should have a "Download Invoice" button on the user dashboard so the administration isn't burdened with emailing financial documents.   
